...
export default function HomePage() {
  ...
  return (
    <main
      className={`mx-auto flex min-h-screen w-full max-w-[1080px] flex-col px-4 sm:px-6 lg:px-8 ${
        isSchedule ? "gap-2.5 pt-3 pb-10" : "gap-6 pt-8 pb-16"
      }`}
    >
      <header
      ...
      <section
        className={
          isSchedule
            ? "flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-[var(--color-text-muted)]"
      ...
      <div
        className={`card-surface flex flex-wrap items-center justify-between ${
          isSchedule ? "gap-1 px-3 py-1" : "gap-3 px-3 py-2"
        }`}
      >

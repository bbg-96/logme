...
export function CalendarDayCell({
  ...
  return (
    <button
      ...
      className={cx(
        "group relative flex h-full flex-col items-center justify-between rounded-2xl border text-left transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500",
    ...
    >
      <div className="flex w-full flex-1 flex-col items-center justify-center gap-1 pb-1">
        ...
        {previewTitle && (
          <p className="max-w-[95%] overflow-hidden text-ellipsis text-center text-[11px] leading-snug text-[var(--color-text-muted)] group-hover:text-[var(--color-text-primary)]">
            {previewTitle}
          </p>
        )}
      </div>
